...
const { getUserSocketsByMac } = require("../socketRegistry");
...
function handleClientEvents(clientSocket, aiSocket, options = {}) {
  ...
  clientSocket.on(EVENT_NAMES.DO_FACTORY_RESET, (mac) => {
    ...
    const targetSockets = getUserSocketsByMac(userId, mac).filter((s) => s.connected);
    if (targetSockets.length > 0) {
      const ids = targetSockets.map((s) => s.id).join(", ");
      logger.log(clientSocket.id, `☢️[Factory Reset] Emitting to socket(s): ${ids}`);
      targetSockets.forEach((s) => {
        logger.log(clientSocket.id, `☢️[Factory Reset] Emitting to socket: ${s.id}`);
        s.emit(EVENT_NAMES.FACTORY_RESET);
      });
      return;
    }
  ...
  clientSocket.on(EVENT_NAMES.USER_AUDIO, (payload) => {
    // logger.log(clientSocket.id, `[Audio Chunk] Received audio chunk of type ${typeof payload} and size ${payload.byteLength || payload.length || 0} bytes`);
