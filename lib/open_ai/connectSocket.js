...
function handleConnection(clientSocket) {
  ...
  const sendReminderTaskToAi = ({ task }) => {
    ...
    aiSocket.sendEvent({
      ...
      response: {
        tool_choice: "none",
        instructions: `Give a gentle and caring reminder in natural English. The first sentence must clearly mention the task: ${task}. Use a warm, supportive tone (not commanding), like a friendly nudge. Keep it brief, do not ask follow-up questions, do not mention current time, and do not call any tool.`,
      },
    });
  };
