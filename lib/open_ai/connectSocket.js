...
function handleConnection(clientSocket) {
  ...
  const sendReminderTaskToAi = ({ task }) => {
    ...
    aiSocket.sendEvent({
      ...
      response: {
        modalities: ["audio", "text"],
        instructions: `Remind the user about the following task. Address them by their name (you already know it from the system prompt). Be warm, brief, and conversational — like a caring friend nudging them. Do not say  sound robotic. Task: ${task}.`,
      },
    });
  };
