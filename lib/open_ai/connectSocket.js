...
function handleConnection(clientSocket) {
  ...
  const shouldConnectAi = user?.connectAi !== false;
  const shouldSendGreeting = user?.getGreetingMsg !== false;
  ...
  const connectWithoutGreeting = async () => {
    await sleepManager.wakeUp({
      ...
      onReady: () => {
        clientSocket.emit(EVENT_NAMES.AI_READY);
      },
    });
  };
  ...
  aiSocket.on("error", (err) => {
    const isExpired =
      err.message?.includes("session_expired") || err.code === "session_expired";

    if (isExpired) {
      logger.log(clientSocket.id, "[OpenAI SDK] Session expired (60 min). Auto-reconnecting...");
      aiSocket.close();
      connectWithoutGreeting().catch((reconnErr) => {
        logger.error(clientSocket.id, `[OpenAI SDK] Auto-reconnect failed: ${reconnErr.message}`);
        clientSocket.emit(EVENT_NAMES.AI_ERROR, { error: "Session expired. Reconnect failed." });
      });
      return;
    }
  ...
  if (!shouldConnectAi) {
    logger.log(clientSocket.id, "[OpenAI SDK] Skipping AI connect because connect_ai=false");
  ...
  handleClientEvents(clientSocket, aiSocket, {
    onWakeUp: async () => {
      await sleepManager.wakeUp({
        ...
        onReady: () => { clientSocket.emit(EVENT_NAMES.AI_READY,) },
      });
      sendInitialGreeting(aiSocket, clientSocket);
    },
