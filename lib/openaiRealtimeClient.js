...
class OpenAIRealtimeClient extends EventEmitter {
  constructor({ apiKey, model }) {
    ...
    if (!model) {
      throw new Error("OPENAI_REALTIME_MODEL is required.");
    }
  ...
  async connect(session) {
    this.realtime = await OpenAIRealtimeWS.create(this.client, { model: this.model });

    this.realtime.on("event", (event) => {
      // Wait for session.created before sending session.update
      if (event.type === "session.created" && session) {
  ...
  close() {
    ...
    try {
      this.realtime.close({ code: 1000, reason: "Client disconnected" });
    } catch {
